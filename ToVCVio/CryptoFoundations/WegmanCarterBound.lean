...
stated over an arbitrary run distribution so that they depend on nothing but the
...
* `combine_pre_post_le`: the two are added under a single budget `q` on the total number of
  queries, which is what gives `q · ε` rather than `2 · q · ε`.
-/
...
says the event `E` observed at the end of the continuation `k` is exactly `m ∈ pre w`. -/
...
/-- Adds the two halves under one budget: the per-run counts satisfy `npre z + npost z ≤ q`
jointly, and `hfloor` (`2⁻¹²⁸ ≤ ε`) lets the pre-challenge term be bounded at rate `ε` too, so
the two counts add under one `q`. Bounding each half by `q` on its own would give `2 · q · ε`. -/
