...
* AXU is assumed of `hash` on the *encoded* domain `D`. A polynomial hash on raw block lists
  is not AXU (leading zero blocks collide); the AEAD's length encoding is what rules such
  collisions out. Instantiate `hash := hash ∘ encode` and `enc := id`.
...
oracle and a decrypt oracle. Spelled inline so that this file depends only on VCVio. -/
...
the challenge ciphertext, sets the flag whenever the tag verifies (never clearing it), and
...
/-- Decrypt step of the always-reject handler: the response is always `none` and the state
change is a pure flag update. -/
...
`forged || ok` on the flag, since `wcAccepts` ignores the entry's pre/post tag. -/
...
/-- The `forged` flag of the instrumented run has the probability of `wcFlag` on the
log-refined run. -/
...
/-- The log-length bound from an arbitrary start state. -/
...
/-- An adversary making at most `q` decrypt queries produces a log of at most `q` entries. -/
theorem wcLogImpl_log_length_le [DecidableEq Cb] {α : Type}
    (hash : K → D → BitVec 128) (enc : A × Cb → D) (H : K) (mask : BitVec 128)
    (padMsg : M → Cb) (oa : OracleComp (wcSpec A M Cb) α) (q : ℕ)
...
run no longer mentions `H` or the mask (dummies `H0`, `0`); the mask survives only in the
observed value, as `hash H X* ^^^ T`. -/
...
/-- `reparam_bind` followed by `hoist_H`. -/
