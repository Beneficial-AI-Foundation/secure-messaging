...
sending the `i`-th most-significant bit to the coefficient of `root p ^ i` (the bit order NIST
uses for GCM), with XOR additivity (`reflect_xor`) and the coordinate expansion (`reflect_apply`).
Monicity is the only hypothesis.
...
/-- `reflect` sends bit-vector XOR to addition in `AdjoinRoot p`. -/
...
/-- `reflect hp x` expanded in the power basis: bit `i` (most-significant first) is the
coefficient of `root p ^ i`. -/
