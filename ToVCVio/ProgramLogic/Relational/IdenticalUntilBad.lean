...
/-!
# Identical-until-bad with a decoupled base monad (upstream candidate)
...
`simulateQ`) is independent of the base monad `OracleComp spec₂` (for an arbitrary `spec₂`). This is
the shape needed whenever a game oracle implementation **fully resolves** the adversary's oracles
into a *different* base monad (e.g. an AEAD game whose encrypt/decrypt oracles resolve down to
`ProbComp = OracleComp unifSpec`), so the adversary-spec and the base-spec genuinely differ and the
VCVio lemma does not apply as stated.

The proof is the *same* induction as the VCVio lemma — only the base monad is threaded as a separate
`spec₂` instead of being forced equal to `spec`. It therefore strictly generalizes the VCVio lemma:
`spec₂ := spec` recovers `tvDist_simulateQ_le_probEvent_output_bad`.

## Status

`tvDist_simulateQ_le_probEvent_output_bad_base` is proved (sorry-free), together with the three
private helpers; it is a mechanical decoupling of an existing VCVio lemma (no new probabilistic
content). TODO(upstream): replace the VCVio lemma's `OracleComp spec` base with an arbitrary
`OracleComp spec₂` **in place** (the existing signature becomes the `spec₂ := spec` instantiation),
then delete this file. Downstream callers needing the `ProbComp` base instantiate at
`spec₂ := unifSpec`.
-/
...
base case (the shape an AEAD-style game hop, whose oracles resolve to `ProbComp`, consumes). -/
