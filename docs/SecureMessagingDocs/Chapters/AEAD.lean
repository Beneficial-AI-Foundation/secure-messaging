...
_AEAD-AES-GCM._ Construction of an AEAD scheme from AES-GCM as specified
in NIST SP 800-38D.
{uses "aead_scheme"}[] is the target interface.
:::

:::theorem "aead_aes_gcm_correctness" (parent := "aead")
_Correctness of AEAD-AES-GCM._ The AES-GCM construction satisfies
{uses "aead_correctness"}[].
:::

:::theorem "aead_aes_gcm_security" (parent := "aead")
_Security of AEAD-AES-GCM._ The AES-GCM construction is secure
...
_AEAD-Encrypt-then-MAC._ Generic construction of an AEAD scheme from a
...
_Correctness of Encrypt-then-MAC._ The EtM construction satisfies
{uses "aead_correctness"}[].
:::

:::theorem "aead_etm_security" (parent := "aead")
_Security of Encrypt-then-MAC._ The EtM construction is secure
in the sense of {uses "aead_security_exp"}[].
:::
