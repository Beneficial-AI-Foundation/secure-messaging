...
How an interactive oracle game is represented with `OracleSpec`, `OracleComp`,
and `QueryImpl`, viewed through the polynomial-functor / free-monad lens.
-/
...
oracle syntax plus an interpreter.
:::
...
The four-layer stack above is what game-based security proofs in VCV-io stand
on. A typical security game has the following shape:
...
Each protocol formalization in this development reuses exactly this stack.
