...
def snapshot_metric_labels(snapshot: dict, kind: str, metric: str) -> set[str]:
    # Read the atom labels recorded behind one metric count, when present.
    if kind == CHART_ALL_KINDS:
        return set().union(
            *(snapshot_metric_labels(snapshot, entry, metric) for entry in HISTORY_KINDS)
        )
    counts = snapshot.get(kind, {})
...
def progress_chart(
    ...
    return f'''
        <article class="progress-chart-card" data-progress-metrics="{metrics_attr}">
          <div class="progress-chart-card-head">
            <h3>{title_text}</h3>
            <p class="progress-chart-hint">Hover a point for snapshot details; click to pin days below, click again or press Esc to unpin</p>
          </div>
