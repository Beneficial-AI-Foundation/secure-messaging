...
encryption-error samples, `d_u = 10` gives the `u`-compression error table,
...
ML-KEM-768 Table 1 failure-rate bound. "Certifies" here means only this: Lean
checks that the exact finite sum `decodeFailureMass .MLKEM768`, after the
`256`-coordinate union bound and the normalization by
`2 * noiseDenominator .MLKEM768`, is at most `2^-164.8`.

NB: this module imports the ML-KEM-512 arithmetic check only to serialize the heavy
