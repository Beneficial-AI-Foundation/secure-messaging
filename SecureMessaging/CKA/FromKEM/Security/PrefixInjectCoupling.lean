...
This file implements the injected-prefix coupling behind the final gap bridge.
...
land in the challenge-bridge shapes.  The public result is the
gap-level bound `cka_injected_honest_gap_le_keygen_swapped_raw_gap`, the
missing hop in the top-level advantage chain.
-/
...
private lemma securityImpl_challA_run_of_not_will [SampleableType K] [DecidableEq K]
...
private lemma securityImpl_challB_run_of_not_will [SampleableType K] [DecidableEq K]
    (kem : KEMScheme ProbComp K PK SK C)
    (hDet : DeterministicDecaps kem)
    (leak : RandLeak kem)
    (gp : CKAScheme.GameParams)
...
private def injectedChallengePrefix [SampleableType K] [DecidableEq K]
    (kem : KEMScheme ProbComp K PK SK C)
    (hDet : DeterministicDecaps kem)
    (leak : RandLeak kem)
    (gp : CKAScheme.GameParams)
...
private def injectedChallengeResume [SampleableType K] [DecidableEq K]
    (kem : KEMScheme ProbComp K PK SK C)
    (hDet : DeterministicDecaps kem)
    (leak : RandLeak kem)
    (gp : CKAScheme.GameParams)
...
challenge query whose `willChallengeA`/`willChallengeB` guard holds. -/
private lemma ckaSecurityFixedBranchWithInjectedChallengeKey_eq_split
    [SampleableType K] [DecidableEq K]
    (kem : KEMScheme ProbComp K PK SK C)
    (hDet : DeterministicDecaps kem)
    (leak : RandLeak kem)
...
private lemma oracleSendAWithChallengePk_run_sendReady
    (kem : KEMScheme ProbComp K PK SK C)
    (gp : CKAScheme.GameParams)
    (pkStar : PK)
...
private lemma oracleSendBWithChallengePk_run_sendReady
    (kem : KEMScheme ProbComp K PK SK C)
    (gp : CKAScheme.GameParams)
    (pkStar : PK)
    (σ : SecurityState K PK SK C) (pk : PK)
...
private lemma injectedChallengePrefix_run_done_of_challengePassed
    [SampleableType K] [DecidableEq K]
    (kem : KEMScheme ProbComp K PK SK C)
    (hDet : DeterministicDecaps kem)
    (leak : RandLeak kem)
...
private lemma challengePrefix_run_done_of_challengePassed
    [SampleableType K] [DecidableEq K]
    (kem : KEMScheme ProbComp K PK SK C)
    (hDet : DeterministicDecaps kem)
    (leak : RandLeak kem)
...
`securityImpl`, so one invariant tracks both runs.  `securityShapeInv` records
the state shape these steps maintain: the pending secret keys pair with the
public keys they will decapsulate against, as key-generation support pairs.
...
private lemma mem_support_encaps_of_encaps_rleak
    (kem : KEMScheme ProbComp K PK SK C)
    (leak : RandLeak kem)
...
private lemma mem_support_keygen_of_keygen_rleak
    (kem : KEMScheme ProbComp K PK SK C)
    (leak : RandLeak kem)
...
private lemma oracleSendAWithChallengeKeyPair_run_eq_of_not_inject
    [SampleableType K] [DecidableEq K]
    (kem : KEMScheme ProbComp K PK SK C)
    (hDet : DeterministicDecaps kem)
    (leak : RandLeak kem)
...
private lemma oracleSendBWithChallengeKeyPair_run_eq_of_not_inject
    [SampleableType K] [DecidableEq K]
    (kem : KEMScheme ProbComp K PK SK C)
    (hDet : DeterministicDecaps kem)
    (leak : RandLeak kem)
...
private lemma oracleSendAWithChallengePk_run_eq_of_not_inject
    [SampleableType K] [DecidableEq K]
    (kem : KEMScheme ProbComp K PK SK C)
    (hDet : DeterministicDecaps kem)
    (leak : RandLeak kem)
...
private lemma oracleSendBWithChallengePk_run_eq_of_not_inject
    [SampleableType K] [DecidableEq K]
    (kem : KEMScheme ProbComp K PK SK C)
    (hDet : DeterministicDecaps kem)
    (leak : RandLeak kem)
...
private lemma injectedPrefix_couples_challengePrefix_A
    [SampleableType K] [DecidableEq K]
    (kem : KEMScheme ProbComp K PK SK C)
    (hDet : DeterministicDecaps kem)
    (hkem : kem.PerfectlyCorrect ProbCompRuntime.probComp)
