...
Correctness of the opportunistic UniKEM sparse continuous key-agreement
construction.  The proof is support based: a protocol/game invariant is
preserved by every correctness oracle, and therefore by every correctness
adversary.  Besides perfect correctness, the module gives a quantitative
interface bounding protocol failure by `q * δ` when each of at most `q`
fresh `Send-B` trials has failure probability at most `δ`.
-/
...
private structure EpochTranscript
...
private def EpochTranscript.key
    {kem : KEMScheme ProbComp K PK SK C} {onoff : kem.OnOffStructure}
...
@[simp, nolint simpNF] private def optionPair {A B : Type} : Option A → Option B → Option (A × B)
...
private lemma oracleRecvA_preserves_reachableInv_of_current
    [DecidableEq K]
    (kem : KEMScheme ProbComp K PK SK C) (onoff : kem.OnOffStructure)
    (hDet : DeterministicDecaps kem)
    (ecEk : ErasureCodePayload PK Sym)
...
For an imperfect KEM, the Boolean below recognizes a sampled current-epoch
trial whose B-side key is not recovered by A's deterministic decapsulation.
The quantitative theorem charges only `Send-B` queries: receives, including
replays, are free once the sampled trial has been checked. -/
...
trial.  Incomplete epochs return `false`. -/
def currentKEMFailure [DecidableEq K]
    (kem : KEMScheme ProbComp K PK SK C) (onoff : kem.OnOffStructure)
    (hDet : DeterministicDecaps kem)
    (s : SCKAScheme.GameState (StA onoff Sym) (StB onoff Sym) K (Message Sym)) : Bool :=
  match s.stA.dkA, s.stA.ct0, s.stB.ct1, s.keyB s.stA.t with
  | some dk, some ct0, some ct1, some key =>
      decide (hDet.decapsDet dk (onoff.split.symm (ct0, ct1)) ≠ some key)
  | _, _, _, _ => false

private lemma currentKEMFailure_eq_false_iff [DecidableEq K]
    (kem : KEMScheme ProbComp K PK SK C) (onoff : kem.OnOffStructure)
    (hDet : DeterministicDecaps kem)
    (s : SCKAScheme.GameState (StA onoff Sym) (StB onoff Sym) K (Message Sym)) :
    currentKEMFailure kem onoff hDet s = false ↔
      CurrentKEMCorrect kem onoff hDet s := by
  unfold currentKEMFailure CurrentKEMCorrect
  cases hdk : s.stA.dkA <;> cases hct0 : s.stA.ct0 <;>
    cases hct1 : s.stB.ct1 <;> cases hkey : s.keyB s.stA.t <;>
    simp [hdk, hct0, hct1, hkey]
  constructor
  · intro h dk ct0 ct1 key hdk' hct0' hct1' hkey'
    subst dk
    subst ct0
    subst ct1
    subst key
...
private def trackedInv [DecidableEq K]
  ...
  p.2 = true ∨
    reachableInv kem onoff ecEk ecCt0 ecCt1 p.1 ∧
      CurrentKEMCorrect kem onoff hDet p.1
...
private lemma trackedCorrectnessImpl_preserves
  ...
  subst z
  rcases hp with hbad | ⟨hreach, hcurrent⟩
  · left
    simp [hbad]
  ...
  · right
    ...
    refine ⟨hreach', ?_⟩
    exact (currentKEMFailure_eq_false_iff kem onoff hDet y.2).mp hfailFalse
...
`SendBFailureBound` is deliberately a protocol-facing premise.  Relating a
concrete KEM's average-case correctness error to this adaptive premise is a
separate reduction. -/
...
-- ANCHOR_END: correctnessFailureLe
  ...
  have hinit : Inv (s₀, false) := by
    ...
    · simpa [s₀, initialGame, initialA, initialB] using
        reachableInv_init kem onoff ecEk ecCt0 ecCt1 hEkPos hCt0Pos
    · simp [CurrentKEMCorrect, s₀, initialGame, initialA, initialB,
        SCKAScheme.initGameState]
  have hmono :
