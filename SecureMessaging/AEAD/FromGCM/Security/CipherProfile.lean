...
One-time GCM calls the block cipher on exactly three separated inputs: `0¹²⁸` (giving the
GHASH key `H`), `J₀` (the tag mask) and the counter chain from `inc₃₂(J₀)` (the GCTR
keystream, one call per message block). `gcmEncrypt_profile`/`gcmDecrypt_profile` rewrite the
algorithms as functions `gcmEncryptSpec`/`gcmDecryptSpec` of these three outputs, which is
where the security proof substitutes random values for them. Distinctness of the inputs is in
`Security/Counter.lean`.
-/
