...
The exact cipher-call profile of one-time GCM: at the all-zero 96-bit IV,
`gcmEncrypt`/`gcmDecrypt` consume exactly three separated cipher outputs, the hash key
`H = CIPH_K(0¹²⁸)`, the tag mask `CIPH_K(J₀)` with `J₀ = 1`, and the GCTR keystream
blocks `CIPH_K` over `counterChain 2 ⌈lenP/128⌉`.

The profile theorems are `gcmEncrypt_profile` and `gcmDecrypt_profile`, specialized to
`gcmOneTimeAEAD` at the end; before them come the single-block keystream identity, the
`J₀ = 1` and `inc₃₂(1) = 2` numeral identities, and the single-block `gctr` mask
identity.
-/
