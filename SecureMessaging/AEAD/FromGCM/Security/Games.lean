...
Every game runs the adversary against an instance of `gcmGameSkeleton`. The chain, in which
the only random ingredient from `game1` on is the tuple `(H, mask, ks)` of block-cipher
outputs GCM uses (GHASH key, tag mask, keystream):
...
(`game4`). The tuple argument is ignored; it is kept so that `game4` has the same
`consumeLazy` shape as `game3`, at which the privacy hop couples the two. -/
@[nolint unusedArguments]
...
`game2` and `game3` differ only in what a decryption query returns when the tag verifies.
To bound their distance by the probability of that event (identical-until-bad: two games
that behave the same until a flag is raised differ by at most the probability of the flag),
both are restated with a `forged` flag set whenever a tag verifies. The Lean names
...
added to the state, set and never cleared once a decryption query's tag verifies. The flag
update does not depend on `b`, so the two variants agree until the flag is set. -/
...
and is eliminated using that `prp.keygen` never fails (`NeverFail`, inferred for every
`ProbComp`). -/
