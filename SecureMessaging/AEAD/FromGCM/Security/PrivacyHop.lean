...
masked by a fresh `mask`, and the pair is jointly uniform on `BitVec L × BitVec 128`
(`evalDist_gcmChallenge_uniform`). The two games are therefore equidistributed
(`game3_eq_game4`), by a per-query coupling (`gcmPrivacy_step`) under the state relation
`gcmPrivacyRel`.
-/
...
`BitVec L × BitVec 128`: given `H` and `ks`, `C = m ^^^ ks` is uniform because `ks` is, and
`mask` is still fresh, so `T` is uniform and independent of `C`. The statement is against the
single product draw; uniformity of `C` and `T` separately would not suffice for the coupling. -/
...
/-- `ODecrypt`: both sides always reject, whatever tuple `consumeLazy` feeds in
...
lazy-sampling cache and `gcmPrivacyRel` forgets it on that side alone. -/
