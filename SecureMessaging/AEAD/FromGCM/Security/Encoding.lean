...
/-- Upper bound on the block count of `gcmEncode ad c` for `c : BitVec L`
(`gcmEncode_length_le`); the degree bound of the GHASH polynomial. -/
...
/-- Distinct domain points give encodings that differ at some position of the *reversed*
block list. Since `ghash h [X₁, …, Xₘ] = X₁·hᵐ ⊕ ⋯ ⊕ Xₘ·h`, reversed index `i` is the
coefficient of `h^(i+1)`, so the difference of two distinct encodings is a nonzero polynomial
of degree `≤ maxBlocks L` with no constant term; this is the form `GhashAXU.lean` needs.
Comparing leading blocks instead would fail: a longer AAD starting with zero blocks can share
them with a shorter one. -/
