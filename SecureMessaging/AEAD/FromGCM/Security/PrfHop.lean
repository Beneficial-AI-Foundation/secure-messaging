...
/-- The PRF distinguisher built from a GCM adversary. It fetches the GHASH key (at `0`), the
tag mask (at `j0 iv`) and the `⌈L/128⌉` keystream blocks (at the counter chain) from its function
oracle, then runs `adv` against `gcmTupleImpl` at that tuple; the tail makes no further
oracle query. Fetching everything up front keeps the query count fixed at `⌈L/128⌉ + 2`,
...
pairwise distinct (`cipherInputs_pairwise_ne`), so every query is fresh. This is where
`ValidMsgLength L` is needed: `inc32` wraps modulo `2³²`, and for a long enough message the
counter chain would repeat a point. -/
...
many independent uniform draws. `hL` makes the query points distinct; without it the
statement is false. -/
lemma prfIdealExp_prfReduction_eq (iv : BitVec 96) (L : ℕ) (hL : ValidMsgLength L)
  ...
  -- matching would have to solve `Range (Sum.inr ?d) =?= BitVec 128` (see
  -- `ToVCVio/CryptoFoundations/PRF.lean`). Term elaboration unifies up to definitional
  -- unfolding.
...
continuation. (VCVio's `evalDist_bind_congr_left` varies the continuation instead.) -/
...
theorem prfReduction_isQueryBoundP (iv : BitVec 96) (L : ℕ)
  ...
  have key : (prfReduction iv L adv).IsQueryBoundP (· matches Sum.inr _)
    ...
    unfold prfReduction
    refine isQueryBoundP_bind (PRFScheme.isQueryBoundP_query_inr _ _ (by omega)) fun h _ => ?_
    refine isQueryBoundP_bind (PRFScheme.isQueryBoundP_query_inr _ _ (by omega))
      fun mask _ => ?_
    refine isQueryBoundP_bind ?_ fun blocks _ => ?_
    · refine (isQueryBoundP_listMapM_const (k := 1)
        (fun x => PRFScheme.isQueryBoundP_query_inr x 1 (by omega)) _).mono ?_
