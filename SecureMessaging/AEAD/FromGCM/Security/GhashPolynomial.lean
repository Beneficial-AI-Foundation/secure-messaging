...
/-- `x¹²⁸ = x⁷ + x² + x + 1` in the quotient; in characteristic 2 the sign of the reduction is
irrelevant. This is the equation the `⊕ gcmReductionConst` step of `gfmul` implements. -/
theorem nistPoly_root_pow :
  ...
  linear_combination h
    - (AdjoinRoot.root nistPoly ^ 7 + AdjoinRoot.root nistPoly ^ 2
        + AdjoinRoot.root nistPoly + 1) * adjoinRoot_two_eq_zero
...
`BitVec 128`: bit `i`, counted from the most significant end, is the coefficient of
`root nistPoly ^ i`. -/
...
/-- The reduction constant `R = 0xE1 <<< 120` has most-significant-first positions `0, 1, 2, 7`
set, so under `reflectN` it reads as `x⁷ + x² + x + 1`, which is `x¹²⁸` in the quotient. -/
theorem reflect_gcmReductionConst :
    reflectN gcmReductionConst
      = AdjoinRoot.root nistPoly ^ 7 + AdjoinRoot.root nistPoly ^ 2
        + AdjoinRoot.root nistPoly + 1 := by
  have h : ∀ i : Fin 128, boolToZMod2 (gcmReductionConst.getMsbD i)
      = if i ∈ ({0, 1, 2, 7} : Finset (Fin 128)) then 1 else 0 := by decide
  rw [reflectN_apply]
  simp only [h, ite_smul, one_smul, zero_smul, Finset.sum_ite_mem, Finset.univ_inter]
  simp [Finset.sum_insert]
  ring

/-! ## One `gfmul` step multiplies by `root`

The `v`-update of `gfmul` right-shifts, which moves every coefficient one degree up,
`xʲ ↦ xʲ⁺¹`, and drops `x¹²⁷`; when the dropped bit was set it XORs in `R`, which by
`reflect_gcmReductionConst` and `nistPoly_root_pow` restores exactly the lost `x¹²⁸` term. -/
...
theorem reflect_gfmulStep (v : BitVec 128) :
  ...
  · have hif : v.getLsbD 0 = true := by rw [hcond]; exact h
    rw [if_pos hif, reflectN_xor, key, reflect_gcmReductionConst, ← nistPoly_root_pow, h,
      boolToZMod2_true, one_smul]
  · have hif : ¬ (v.getLsbD 0 = true) := by rw [hcond]; exact h
    have h127 : v.getMsbD 127 = false := by simpa using h
    rw [if_neg hif, key, h127, boolToZMod2_false, zero_smul, add_zero]
...
theorem reflect_gfmul_aux (x y : BitVec 128) (k : ℕ) :
  ...
  | succ k ih =>
    ...
    · change reflectN (if x.getMsbD k then p.1 ^^^ p.2 else p.1)
      ...
      · rw [if_pos h, reflectN_xor, ihz, ihv, h, boolToZMod2_true, one_smul]
        ring
      · rw [if_neg h]
        rw [Bool.not_eq_true] at h
        rw [ihz, h, boolToZMod2_false, zero_smul, zero_mul, add_zero]
...
/-- The polynomial `B₁·Xᵐ + ⋯ + Bₘ·X` whose value at `h` is `ghash h [B₁, …, Bₘ]`. Its constant
