...
The GCM handlers are identified with the generic Wegman–Carter handler of
`ToVCVio/CryptoFoundations/WegmanCarter.lean` (`gcmInstImpl_eq_wcInstImpl`), and the two games
...
/-- The GCM instrumented handler is the generic Wegman–Carter handler with `enc := id`,
`hash := fun H p => ghash H (gcmEncode p.1 p.2)` and `(· ^^^ ks)` as both pad and unpad. The
encoding sits inside `hash` rather than `enc` because GHASH on raw block lists is not AXU
(`Axu.lean`). Stated on a destructured tuple, since `let (h, mask, ks) := a` does not reduce
for a variable `a`. -/
theorem gcmInstImpl_eq_wcInstImpl (L : ℕ) (h mask : BitVec 128) (ks : BitVec L)
  ...
  funext t
  -- Plain `rfl` is stuck on two distinct matcher auxiliaries; case-splitting the challenge
  -- slot lets both iota-reduce.
...
`probEvent_wcInst_forge_le` transported along `gcmInstImpl_eq_wcInstImpl`. -/
...
/-- At a fixed tuple, the total-variation distance between the always-reject and the live run is
at most the probability that the always-reject run raises its `forged` flag. -/
