...
(GHASH key, tag mask, `n` keystream blocks), giving `(n + 2)(n + 1) / 2¹²⁹`. Because the list
is fixed and its points are distinct (`cipherInputs_pairwise_ne`, which needs `hL`), the
random permutation is only ever seen through its values on that list, a without-replacement
sample; the generic bound is `tvDist_map_uniformPerm_mapM_const_uniform_le`
(`ToVCVio/EvalDist/UniformInjection.lean`).
...
`q(q − 1) / 2¹²⁹` with `q = ⌈L/128⌉ + 2`. The two advantages share the real experiment
